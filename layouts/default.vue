    ...
    <div class="self-center rounded-full hover:bg-gray-500 w-14 ml-3 h-12 hover:bg-opacity-20 transition-colors duration-200 flex items-center justify-center select-none max-sm:z-50" role="button" tabindex="0" @click="switch_sidebar">
      ...
      <!-- Lab menu -->
        <div class="side-butt-mobile mt-2 rounded-xl overflow-hidden max-sm:mx-0">
          ...
          <div class="transition-all duration-500 ease-out overflow-hidden bg-gray-900 bg-opacity-50 select-none"
            ...
            <div class="flex flex-col bg-gray-900 border-gray-600 text-center space-y-1 py-2 px-3" >
              ...
              <NuxtLink to="/games/ttt" @click="sidebar = false; burger = false" 
                        class="relative group flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-300 hover:bg-blue-500 hover:shadow-md hover:scale-[1.02] hover:translate-x-2 border-l-2 border-transparent hover:border-blue-300">
                <div class="w-2 h-2 bg-blue-400 rounded-full transition-transform duration-300 group-hover:scale-150"></div>
                <span class="font-medium">Tic Tac Toe</span>
                <span class="nav-span rounded-lg"></span>
              </NuxtLink>
              <NuxtLink to="/games/flappybird" @click="sidebar = false; burger = false" 
                        class="relative group flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-300 hover:bg-blue-500 hover:shadow-md hover:scale-[1.02] hover:translate-x-2 border-l-2 border-transparent hover:border-blue-300">
                <div class="w-2 h-2 bg-blue-400 rounded-full transition-transform duration-300 group-hover:scale-150"></div>
                <span class="font-medium">Flappy Bird</span>
                <span class="nav-span rounded-lg"></span>
              </NuxtLink>
              <NuxtLink to="/games/2048" @click="sidebar = false; burger = false" 
                        class="relative group flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-300 hover:bg-blue-500 hover:shadow-md hover:scale-[1.02] hover:translate-x-2 border-l-2 border-transparent hover:border-blue-300">
                <div class="w-2 h-2 bg-blue-400 rounded-full transition-transform duration-300 group-hover:scale-150"></div>
        ...
        <div class="side-butt-mobile mt-2 rounded-xl overflow-hidden max-sm:mx-0">
          ...
          <div class="relative side-butt flex items-center justify-start gap-3 px-4 py-3 rounded-xl transition-all duration-300 group cursor-pointer select-none max-sm:rounded-none max-sm:border-b max-sm:border-gray-600"
              :class="{'bg-blue-600 shadow-lg scale-[1.02] border-l-4 border-blue-400': submenu2, 'hover:bg-gray-800 hover:scale-[1.02] border-l-4 border-transparent': !submenu}"
              @click="switch_submenu2" @mouseenter="handleMouseEnter2" @mouseleave="handleMouseLeave2" role="button" tabindex="0">
            <svg class="w-5 h-5 transition-transform duration-300" :class="{'rotate-90 scale-110': submenu2, 'group-hover:scale-110': !submenu2}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6.5 15l1.5-2h8l1.5 2M8 13v-2m8 2v-2m-5 0h2m-7 4a5 5 0 01-4-2V9a4 4 0 014-4h10a4 4 0 014 4v6a5 5 0 01-4 2H6z" />
            </svg>

            <span class="font-medium transition-all duration-300" :class="{'translate-x-1': submenu2, 'group-hover:translate-x-1': !submenu2}">Labs</span>
            <svg class="w-4 h-4 ml-auto transition-transform duration-300" :class="{'rotate-180': submenu2}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
            </svg>
          </div>
          
          <div class="transition-all duration-500 ease-out overflow-hidden bg-gray-900 bg-opacity-50 select-none"
              :class="{'max-h-0': !submenu2, 'max-h-64': submenu2}" @mouseleave="closeWithDelay2" @mouseenter="cancelClose" >
            <div class="flex flex-col bg-gray-900 border-gray-600 text-center space-y-1 py-2 px-3" >
              <NuxtLink to="/lab3" @click="sidebar = false; burger = false" 
                        class="relative group flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-300 hover:bg-blue-500 hover:shadow-md hover:scale-[1.02] hover:translate-x-2 border-l-2 border-transparent hover:border-blue-300">
                <div class="w-2 h-2 bg-blue-400 rounded-full transition-transform duration-300 group-hover:scale-150"></div>
...
const submenu2 = ref<boolean>(false);
const switch_submenu2 = () => {submenu2.value = !submenu2.value;};
const closeWithDelay2 = () => {closeTimeout = setTimeout(() => {submenu2.value = false; }, 350);};
const openWithDelay2 = () => {openTimeout = setTimeout(() => {submenu2.value = true; }, 150);};
const cancelOpen2 = () => {if (openTimeout) {clearTimeout(openTimeout);openTimeout = null;}};
const handleMouseLeave2 = () => {if (submenu2.value) {closeWithDelay2();} else {cancelOpen2();}};
const cancelClose2 = () => {if (closeTimeout) {clearTimeout(closeTimeout);closeTimeout = null;}};

const handleMouseEnter2 = () => {cancelClose2();openWithDelay2();};
